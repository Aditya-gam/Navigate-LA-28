    ...
    switch (activeSubTab) {
      case "attractions": {
        ...
        const attractionsData =
          data.attractions?.popular_attractions.map((attraction) => ({
      ...
      case "demographics": {
        ...
        const demographicsData = data.demographics
          ? {
              age: {
                reviews: Object.entries(data.demographics.age_groups).map(
                  ([age_group, count]) => ({
                    age_group,
                    total_reviews: count,
                ...
                bus_usage: Object.entries(data.demographics.age_groups).map(
                  ([age_group, count]) => ({
                    age_group,
                    total_trips: count,
              ...
              country: {
                reviews: Object.entries(data.demographics.countries).map(
                  ([country, count]) => ({
                    country,
                    total_reviews: count,
                    avg_rating: 4.0, // Default value
                ...
                bus_usage: Object.entries(data.demographics.countries).map(
                  ([country, count]) => ({
                    country,
                    total_trips: count,
                    active_riders: Math.floor(count * 0.6), // Estimate
      ...
      case "bus-routes": {
        ...
        const busRoutesData = data["bus-routes"]
          ? {
              line_popularity: data["bus-routes"].popular_routes.map(
                (route) => ({
              ...
              frequent_routes: data["bus-routes"].popular_routes.map(
                (route) => ({
                  line: route.route_number,
      ...
      case "popular-stops": {
        ...
        const popularStopsData =
          data["popular-stops"]?.popular_stops.map((stop) => ({
